...
export function ensureOrtRuntime(
  url = 'https://cdn.jsdelivr.net/npm/onnxruntime-web@1.26.0/dist/ort.webgpu.min.js',
) {
  ...
  ortLoadPromise = new Promise((resolve, reject) => {
    ...
    script.addEventListener('error', () => reject(new Error(`Failed to load ONNX Runtime Web from ${url}.`)), { once: true });
...
function requireOrt() {
  if (!globalThis.ort) {
    throw new Error('ONNX Runtime Web is not initialized.');
  }
...
function sourceDimensions(source) {
  ...
  if (!(width > 0 && height > 0)) throw new Error('The camera or image has no readable dimensions.');
  return { width, height };
}
...
export function prepareLetterboxInput(source, size = PALM_SIZE, canvas = null) {
  ...
  return {
    data: imageDataToNhwcFloat(imageData),
    transform: { inputSize: size, sourceWidth: width, sourceHeight: height, scale, padX, padY },
    canvas: output,
  };
}
...
export function prepareRoiInput(source, roi, size = LANDMARK_SIZE, canvas = null) {
  ...
  return { data: imageDataToNhwcFloat(context.getImageData(0, 0, size, size)), canvas: output };
}

async function modelBytes(source) {
  if (source instanceof File || source instanceof Blob) return new Uint8Array(await source.arrayBuffer());
  return source;
}

function backendLabel(executionProvider, graphCapture) {
  return `${executionProvider}${graphCapture ? ' + graph capture' : ''}`;
}

export class OrtSessionFactory {
  constructor({ backend = 'auto', wasmPaths = 'https://cdn.jsdelivr.net/npm/onnxruntime-web@1.26.0/dist/' } = {}) {
  ...
  async create(source, label) {
    ...
    const canUseWebGpu = Boolean(navigator.gpu) && this.backend !== 'wasm';
    const attempts = [];
    if (canUseWebGpu) {
      attempts.push({ executionProviders: ['webgpu'], enableGraphCapture: true, graphCapture: true });
      attempts.push({ executionProviders: ['webgpu'], enableGraphCapture: false, graphCapture: false });
    }
    if (this.backend !== 'webgpu') {
      attempts.push({ executionProviders: ['wasm'], enableGraphCapture: false, graphCapture: false });
    }
    if (attempts.length === 0) throw new Error('WebGPU was required, but this browser does not expose navigator.gpu.');
    ...
    for (const attempt of attempts) {
      try {
        const session = await ort.InferenceSession.create(resolvedSource, {
          ...
          enableGraphCapture: attempt.enableGraphCapture,
        });
        const executionProvider = attempt.executionProviders[0];
        const diagnostic = {
          label,
          backend: backendLabel(executionProvider, attempt.graphCapture),
          inputs: session.inputNames,
          outputs: session.outputNames,
        };
        this.sessionDiagnostics.push(diagnostic);
        return { session, executionProvider, graphCapture: attempt.graphCapture, diagnostic };
      } catch (error) {
        lastError = error;
        this.sessionDiagnostics.push({
          label,
          backend: backendLabel(attempt.executionProviders[0], attempt.graphCapture),
          error: error instanceof Error ? error.message : String(error),
        });
      }
    }
    throw new Error(`${label} failed to initialize: ${lastError instanceof Error ? lastError.message : lastError}`);
  }
}

function identifyLandmarkOutputs(outputs, outputNames = Object.keys(outputs)) {
  const ordered = outputNames.map((name) => [name, outputs[name]]).filter(([, tensor]) => tensor);
  ...
  if (vectors.length < 1 || scalars.length < 1) {
    const signature = ordered.map(([name, tensor]) => `${name}:${tensor.dims.join('x')}`).join(', ');
  ...
  const handedness = scalars.find(([name]) => /hand|left|right|lr/i.test(name) && name !== score[0])
    ?? scalars.find((entry) => entry !== score)
    ?? null;
  return {
    landmarks: screen[1],
...
function cloneWorldLandmarks(tensor) {
  ...
  for (let index = 0; index < tensor.data.length; index += 3) {
    result.push({ x: tensor.data[index], y: tensor.data[index + 1], z: tensor.data[index + 2] });
  }
  return result;
}

export class WebGpuHandTracker {
  constructor({ manifest, backend = 'auto', maxHands = 1, detectorInterval = 8, scoreThreshold = 0.55 } = {}) {
  ...
  diagnostics() {
    return {
      backend: this.detector?.executionProvider === 'webgpu' && this.landmark?.executionProvider === 'webgpu'
        ? 'webgpu'
        : 'wasm',
      graphCapture: Boolean(this.detector?.graphCapture && this.landmark?.graphCapture),
  ...
  async #detect(source) {
    ...
    const tensor = new ort.Tensor('float32', prepared.data, [1, PALM_SIZE, PALM_SIZE, 3]);
    const outputs = await this.detector.session.run({ [inputName]: tensor });
  ...
  async #landmarksForRoi(source, roi) {
    ...
    const tensor = new ort.Tensor('float32', prepared.data, [1, LANDMARK_SIZE, LANDMARK_SIZE, 3]);
    const outputs = await this.landmark.session.run({ [inputName]: tensor });
    ...
    const points = projectLandmarksFromRoi(decoded.landmarks.data, roi, LANDMARK_SIZE);
    return {
      score: decoded.score,
  ...
  async infer(source) {
    ...
    const shouldDetect = this.tracks.length === 0 || this.frameIndex % this.detectorInterval === 0;
    ...
    return {
      ...
      graphCapture: this.diagnostics().graphCapture,
      timing: {
